...
impl TryFrom<PrivKey> for G2CompressedPoint {
    ...
    fn try_from(value: PrivKey) -> Result<G2CompressedPoint, Self::Error> {
        ...
        let secret_key = Fr::deserialize_compressed(&pk[..]).map_err(|_| BLSError::SecretKeyError)?;

        let g2_public_key = G2Affine::generator() * secret_key;

        let mut g2_public_key_bytes = [0u8;64];

        g2_public_key.serialize_compressed(&mut &mut g2_public_key_bytes[..]).map_err(|_| BLSError::G2PointCompressionError)?;
...
impl TryFrom<PrivKey> for G2Point {
    ...
    fn try_from(value: PrivKey) -> Result<G2Point, Self::Error> {
        Ok(G2Point(alt_bn128_g2_decompress(&G2CompressedPoint::try_from(value)?.0).map_err(|_| BLSError::G2PointDecompressionError)?))
    }
}
...
impl TryFrom<G2Point> for G2CompressedPoint {
    ...
    fn try_from(value: G2Point) -> Result<Self, Self::Error> {    
        Ok(G2CompressedPoint(alt_bn128_g2_compress(&value.0).map_err(|_| BLSError::G2PointCompressionError)?))
    }
}
...
impl TryFrom<G2CompressedPoint> for G2Point {
    ...
    fn try_from(value: G2CompressedPoint) -> Result<Self, Self::Error> {  
        Ok(G2Point(alt_bn128_g2_decompress(&value.0).map_err(|_| BLSError::G2PointDecompressionError)?))
    }
}

#[cfg(test)]
mod tests {
    use crate::{g1_point::G1CompressedPoint, g2_point::{ G2CompressedPoint, G2Point }, privkey::PrivKey, schemes::sha256_normalized::Sha256Normalized};
    ...
    fn signature_verification() {
        ...
        assert!(pubkey.verify_signature::<Sha256Normalized, &str>(signature_compressed, "sample").is_ok());
    }
}